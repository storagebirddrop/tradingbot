#!/usr/bin/env python3
"""
Phase 1C: Strategy Selection & Implementation
Final strategy selection with implementation parameters and deployment plan
"""
...
# Selected strategy: ATR Breakout (Long)
SELECTED_STRATEGY = "ATR Breakout (Long)"

# Implementation parameters based on Phase 1B results and debugging
IMPLEMENTATION_PARAMS = {
    'strategy_name': 'ATR Breakout (Long)',
    'position_type': 'long',
    'stop_loss_pct': 0.02,  # 2% stop loss
    'take_profit_pct': 0.06,  # 6% take profit (3:1 risk-reward)
    'max_holding_periods': 15,  # 60 hours max (4h * 15)
    'risk_per_trade': 0.01,  # 1% per trade
    'max_portfolio_exposure': 0.25,  # 25% max total exposure
    'atr_multiplier': 1.0,  # ATR multiplier for breakout (reduced from 1.5)
    'volume_ratio_threshold': 1.5,  # Volume confirmation
    'rsi_threshold': 50,  # Not oversold
    'adx_threshold': 25,  # Strong trend requirement
    'lookback_periods': 10,  # Recent high lookback
}
...
def calculate_atr_breakout_indicators(df):
    """Calculate indicators for ATR Breakout strategy"""
    if df.empty:
        return df
    ...
    # Additional indicators for ATR Breakout
    ...
    df_ind['recent_high'] = df_ind['high'].rolling(window=IMPLEMENTATION_PARAMS['lookback_periods']).max()
...
def atr_breakout_strategy(df, params=None):
    """Final ATR Breakout strategy implementation"""
    if params is None:
    ...
    for i in range(15, len(df)):  # Need ATR calculation
        ...
        breakout_level = recent_high + (sig['atr'] * params['atr_multiplier'])
        
        # Entry conditions
        entry_conditions = (
            sig['close'] > breakout_level and  # Price breaks above recent high
            sig['volume_ratio'] > params['volume_ratio_threshold'] and  # Strong volume
            sig['rsi'] > params['rsi_threshold'] and  # Not oversold
            sig['adx'] > params['adx_threshold']  # Strong trend
        )
...
def final_validation():
    ...
    for period_name, period_info in MARKET_PERIODS.items():
        ...
        for symbol in SYMBOLS:
            ...
            df_ind = calculate_atr_breakout_indicators(df)
            
            # Test strategy
            trades = atr_breakout_strategy(df_ind)
...
def generate_implementation_plan(validation_result):
    ...
    print(f"  Risk-Reward: 1:3 (2% SL, 6% TP)")
    ...
    print(f"\n🔧 ENTRY CONDITIONS:")
    print(f"  • Price breaks above {params['lookback_periods']}-period high + (ATR × {params['atr_multiplier']})")
    print(f"  • Volume ratio > {params['volume_ratio_threshold']} (strong volume)")
    print(f"  • RSI > {params['rsi_threshold']} (not oversold)")
    print(f"  • ADX > {params['adx_threshold']} (strong trend)")
    ...
    print(f"  □ Update strategy.py with ATR Breakout logic")
...
def main():
    ...
    try:
        ...
        print(f"\n🎉 PHASE 1C COMPLETE!")
        print(f"✅ Strategy selection and implementation plan complete")
